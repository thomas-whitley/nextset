...
import { ArrowLeftRight, Info, Trash2 } from 'lucide-react-native';
import Colors from '@/constants/Colors';
import { spacing, touch, radius } from '@/constants/theme';
import DragDismissSheet from '@/components/gestures/DragDismissSheet';

type Props = { visible: boolean; exerciseName: string; onDismiss: () => void; onHowTo: () => void; onSwap: () => void; onRemove: () => void };

/** The ⋯ menu on an exercise card (spec R9). Rare actions, one tap deeper, 64dp rows. */
export default function ExerciseActionsSheet({ visible, exerciseName, onDismiss, onHowTo, onSwap, onRemove }: Props) {
  ...
  return (
    <DragDismissSheet visible={visible} onDismiss={onDismiss}>
      <View style={[styles.body, { paddingBottom: spacing.lg + insets.bottom }]}>
        <Text style={styles.heading} numberOfLines={2}>{exerciseName}</Text>
        <Row icon={<Info size={24} color={Colors.light.primary} />} title="How to do it" sub="Muscles worked and step-by-step tips" onPress={onHowTo} />
        <Row icon={<ArrowLeftRight size={24} color={Colors.light.primary} />} title="Swap exercise" sub="Similar ones first, or pick any" onPress={onSwap} />
        <Row icon={<Trash2 size={24} color={Colors.light.error} />} title="Remove from this workout" sub="You can undo for a few seconds" onPress={onRemove} danger />
